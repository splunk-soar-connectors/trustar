...
class TrustarConnector(BaseConnector):
    ...
    def _make_rest_call_helper(self, endpoint, action_result, headers=None, params=None, data=None, json=None,
        ...
        if phantom.is_fail(ret_val):
            ...
            if error_message and (
                    consts.TRUSTAR_INVALID_TOKEN_MESSAGES[0] in error_message or consts.TRUSTAR_INVALID_TOKEN_MESSAGES[1] in error_message):
    ...
    def _get_indicator_summary(self, param):
        ...
        ret_val, indicator_summaries = self._paginate_without_cursor(action_result, consts.TRUSTAR_INDICATORS_SUMMARY_ENDPOINT, body=values,
            params=params)
    ...
    def _submit_report(self, param):
        ...
        if distribution_type == 'ENCLAVE':
            ...
            if self._config_enclave_ids:
                ...
                config_enclave_id_list = list(filter(lambda x: x.strip(), [config_enclave_id.strip()
                    for config_enclave_id in config_enclave_id_list]))
