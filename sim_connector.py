...
import sim_consts as consts

# Dictionary containing details of possible HTTP error codes in API Response
ERROR_RESPONSE_DICT = {
    consts.SIM_REST_RESP_BAD_REQUEST: consts.SIM_REST_RESP_BAD_REQUEST_MSG,
    consts.SIM_REST_RESP_UNAUTHORIZED: consts.SIM_REST_RESP_UNAUTHORIZED_MSG,
    consts.SIM_REST_RESP_RESOURCE_NOT_FOUND: consts.SIM_REST_RESP_RESOURCE_NOT_FOUND_MSG,
    consts.SIM_REST_RESP_TOO_LONG: consts.SIM_REST_RESP_TOO_LONG_MSG,
    consts.SIM_REST_RESP_INTERNAL_SERVER_ERROR: consts.SIM_REST_RESP_INTERNAL_SERVER_ERROR_MSG,
    consts.SIM_REST_RESP_GATEWAY_TIMEOUT: consts.SIM_REST_RESP_GATEWAY_TIMEOUT_MSG,
    consts.SIM_REST_TOO_MANY_REQUESTS: consts.SIM_REST_TOO_MANY_REQUESTS_MSG,
}
...
class SIMConnector(BaseConnector):
    ...
    Splunk Intelligence Management and helper methods required to run the actions.
    """

    def __init__(self):

        # Calling the BaseConnector's init function
        super(SIMConnector, self).__init__()
        self._url = None
    ...
    def initialize(self):
        ...
        self._url = config[consts.SIM_CONFIG_URL].strip("/")
        self._config_enclave_ids = config.get(consts.SIM_CONFIG_ENCLAVE_IDS)
        self._client_id = config[consts.SIM_CONFIG_CLIENT_ID]
        self._client_secret = config[consts.SIM_CONFIG_CLIENT_SECRET]

        ret_val, self._max_wait_time = self._validate_integer(
            self, config.get(consts.SIM_CONFIG_WAIT_TIME, consts.SIM_DEFAULT_MAX_WAIT_TIME), "max wait time"
        )
        ...
        if not isinstance(self._app_state, dict):
            ...
            self._app_state = {"app_version": self.get_app_json().get("app_version")}

        self._access_token = self._app_state.get(consts.SIM_OAUTH_TOKEN_STRING, {}).get(consts.SIM_OAUTH_ACCESS_TOKEN_STRING)
        # Custom validation for IP address
        self.set_validator(consts.SIM_HUNT_IP_PARAM, self._is_ip)
    ...
    def _is_ip(self, cidr_ip_address):
        ...
        try:
            ...
            self.debug_print(consts.SIM_IP_VALIDATION_FAILED, e)
            return False

        # Validate IP address
        if not (phantom.is_ip(ip_address) or _is_ipv6(ip_address)):
            self.debug_print(consts.SIM_IP_VALIDATION_FAILED)
            return False
        ...
        if parameter is not None:
            try:
                if not float(parameter).is_integer():
                    return action_result.set_status(phantom.APP_ERROR, consts.SIM_VALID_INT_MSG.format(param=key)), None

                parameter = int(parameter)
            except Exception as ex:
                self.debug_print("Exception occurred in {}: {}".format(self.get_action_identifier(), ex))
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_VALID_INT_MSG.format(param=key)), None

            if parameter < 0:
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_NON_NEG_INT_MSG.format(param=key)), None
            if not allow_zero and parameter == 0:
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_NON_NEG_NON_ZERO_INT_MSG.format(param=key)), None

        return phantom.APP_SUCCESS, parameter

    def _make_rest_call_helper(
        self, endpoint, action_result, headers=None, params=None, data=None, json=None, method="get", timeout=None, auth=None
    ):
        """
        ...
        if not headers:
            headers = {}
        headers.update({"Authorization": consts.SIM_AUTHORIZATION_HEADER.format(token=self._access_token)})
        ...
        if phantom.is_fail(ret_val):
            ...
            if error_message and consts.SIM_REST_TOO_MANY_REQUESTS_MSG in error_message:
            ...
            if error_message and (
                (consts.SIM_INVALID_TOKEN_MESSAGES[0] in error_message) or (consts.SIM_INVALID_TOKEN_MESSAGES[1] in error_message)
            ):
                self.debug_print(
                    "Refreshing Splunk Intelligence Management API and re-trying request to [{}] "
    ...
    def _make_rest_call(self, endpoint, action_result, headers=None, params=None, data=None, json=None, method="get", timeout=None, auth=None):
        ...
        try:
            ...
            self.debug_print(consts.SIM_ERR_API_UNSUPPORTED_METHOD.format(method=method))
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_API_UNSUPPORTED_METHOD, method=method), response_data
        except Exception as e:
            self.debug_print(consts.SIM_EXCEPTION_OCCURRED, e)
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(phantom.APP_ERROR, consts.SIM_EXCEPTION_OCCURRED, e), response_data

        try:
            # For all actions
            if auth is None:
                response = request_func(
                    "{base_url}{endpoint}".format(base_url=self._url, endpoint=endpoint),
                    params=params,
                    headers=headers,
                    data=data,
                    json=json,
            ...
            self.debug_print(consts.SIM_ERR_SERVER_CONNECTION, e)
            # Set the action_result status to error, the handler function will most probably return as is
            return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_SERVER_CONNECTION, e), response_data
        ...
        try:
            ...
            if self.get_action_identifier() == "submit_report":
                if endpoint == consts.SIM_GENERATE_TOKEN_ENDPOINT:
            ...
            msg_string = consts.SIM_ERR_JSON_PARSE.format(raw_text=response.text)
        ...
        if response.status_code in ERROR_RESPONSE_DICT:
            ...
            self.debug_print(consts.SIM_ERR_FROM_SERVER.format(status=response.status_code, detail=message))
            # Set the action_result status to error, the handler function will most probably return as is
            return (
                action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_FROM_SERVER, status=response.status_code, detail=message),
                response_data,
            )

        # In case of success scenario
        if response.status_code == consts.SIM_REST_RESP_SUCCESS:
            ...
            self.debug_print(consts.SIM_UNEXPECTED_RESPONSE.format(response=response_data))
            return action_result.set_status(phantom.APP_ERROR, consts.SIM_UNEXPECTED_RESPONSE.format(response=response_data)), response_data

        # If response code is unknown
        message = consts.SIM_REST_RESP_OTHER_ERROR_MSG
    ...
    def _paginate_without_cursor(self, action_result, endpoint, body, params=None):
        ...
        page_details = {"pageSize": consts.SIM_PAGE_SIZE, "pageNumber": consts.SIM_PAGE_NUMBER}
        if not params:
            params = {}
        ...
        while True:
            ...
            if cursor is None:
                ...
                if total_results is None:
                    return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_MISSING_FIELD.format(field="totalItems")), None
            ...
            if not cursor:
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_MISSING_FIELD.format(field="nextCursor")), None
    ...
    def _generate_api_token(self, action_result):
        ...
        status, response = self._make_rest_call(
            consts.SIM_GENERATE_TOKEN_ENDPOINT,
            action_result,
            method="post",
            data=data,
            timeout=timeout,
        ...
        if phantom.is_fail(status):
            ...
            self._app_state.pop(consts.SIM_OAUTH_TOKEN_STRING, {})
            return action_result.get_status()

        # Get access token
        self._access_token = response.get(consts.SIM_OAUTH_ACCESS_TOKEN_STRING)

        # Validate access token
        if not self._access_token:
            # Failed to generate new token. Delete the previously generated token in case the credentials are changed.
            self._app_state.pop(consts.SIM_OAUTH_TOKEN_STRING, {})
            self.debug_print(consts.SIM_TOKEN_GENERATION_ERR)
            return action_result.set_status(phantom.APP_ERROR, consts.SIM_TOKEN_GENERATION_ERR)

        self._app_state[consts.SIM_OAUTH_TOKEN_STRING] = response
    ...
    def _test_asset_connectivity(self, param):
        ...
        self.save_progress(consts.SIM_CONNECTION_TEST_MSG)
        ...
        if phantom.is_fail(token_generation_status):
            ...
            action_result.set_status(phantom.APP_ERROR, consts.SIM_TEST_CONNECTIVITY_FAIL)
            return action_result.get_status()

        self.save_progress(consts.SIM_TEST_CONNECTIVITY_PASS)
        return action_result.set_status(phantom.APP_SUCCESS)
    ...
    def _hunt_correlated_reports(self, action_result, ioc_to_hunt):
        ...
        # Make REST call
        return self._make_rest_call_helper(consts.SIM_HUNT_ACTIONS_ENDPOINT, action_result, params=params)

    def _hunt_ioc(self, param):
        """Get list of all Splunk Intelligence Management incident report IDs that correlate with the provided IOC.
        ...
        ioc = param[consts.SIM_HUNT_IOC_PARAM]
        ...
        if not response:
            return action_result.set_status(phantom.APP_SUCCESS, consts.SIM_REASON_FOR_REPORT_UNAVAILABILITY)
    ...
    def _hunt_ip(self, param):
        """Get list of all Splunk Intelligence Management incident report IDs that correlate with the provided IP.
        ...
        # Get mandatory parameters
        ip = param[consts.SIM_HUNT_IP_PARAM]
    ...
    def _hunt_url(self, param):
        """Get list of all Splunk Intelligence Management incident report IDs that correlate with the provided URL.
        ...
        # Get mandatory parameters
        url = param[consts.SIM_HUNT_URL_PARAM]
    ...
    def _hunt_file(self, param):
        """Get list of all Splunk Intelligence Management incident report IDs that correlate with the provided hash.
        ...
        # Get mandatory parameters
        file_param = param[consts.SIM_HUNT_FILE_PARAM]
    ...
    def _hunt_email(self, param):
        """Get list of all Splunk Intelligence Management incident report IDs that correlate with the provided email.
        ...
        # Get mandatory parameters
        email = param[consts.SIM_HUNT_EMAIL_PARAM]
    ...
    def _hunt_cve(self, param):
        """Get list of all Splunk Intelligence Management incident report IDs that correlate with the provided
        ...
        # Get mandatory parameters
        cve_number = param[consts.SIM_HUNT_CVE_PARAM]
    ...
    def _hunt_malware(self, param):
        ...
        # Get mandatory parameters
        malware = param[consts.SIM_HUNT_MALWARE_PARAM]
    ...
    def _hunt_registry_key(self, param):
        ...
        # Get mandatory parameters
        registry_key = param[consts.SIM_HUNT_REGISTRY_KEY_PARAM]
    ...
    def _hunt_bitcoin_address(self, param):
        ...
        # Get mandatory parameters
        bitcoin_address = param[consts.SIM_HUNT_BITCOIN_ADDRESS_PARAM]
    ...
    def _list_emails(self, param):
        """Return a list of emails submitted to Splunk Intelligence Management's phishing triage
        ...
        if start_time:
            try:
                ...
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_TIME_PARSE.format(value_error=e))
        ...
        if pes:
            try:
                ...
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_PRIORITY_EVENT_SCORES)
            body["priorityEventScore"] = pes_list

        if status:
            try:
                status_list = [x.strip() for x in status.split(",")]
                if not all(x in consts.SIM_STATUSES for x in status_list):
                    raise ValueError
            except ValueError:
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_STATUSES)
            body["status"] = status_list

        if enclave_ids:
            body["enclaveIds"] = [x.strip() for x in enclave_ids.split(",")]

        ret_val, response = self._paginate(action_result, consts.SIM_PHISHING_SUBMISSIONS_ENDPOINT, body, "emails_found")
    ...
    def _indicator_reputation(self, param):
        ...
        body = {"queryTerm": query_term, "from": from_time, "to": to_time}
        ...
        ret_val, response = self._paginate(
            action_result, consts.SIM_ENRICH_INDICATOR_ENDPOINT, body, "indicators_found", limit, page_size=consts.SIM_PAGE_SIZE_API_2
        )
    ...
    def _get_indicator_metadata(self, param):
        ...
        if indicator_type:
            ...
            if len(indicator_type) != len(values):
                return action_result.set_status(
                    phantom.APP_ERROR,
                    "Length of 'indicator type' and 'indicator value' parameter should be same. \
                    {}".format(
                        consts.SIM_LESS_INDICATOR_TYPE if len(indicator_type) < len(values) else consts.SIM_LESS_VALUE
                    ),
                )
        ...
        resp_status, response = self._make_rest_call_helper(
            consts.SIM_INDICATORS_METADATA_ENDPOINT, action_result, params=params, json=indicator_list, method="post"
        )
    ...
    def _get_indicator_summary(self, param):
        ...
        ret_val, indicator_summaries = self._paginate_without_cursor(
            action_result, consts.SIM_INDICATORS_SUMMARY_ENDPOINT, body=values, params=params
        )
    ...
    def _list_indicators(self, param):
        """Return a list of indicators extracted from Splunk Intelligence Management's phishing triage submissions
        ...
        ret_val, response = self._paginate(action_result, consts.SIM_PHISHING_INDICATORS_ENDPOINT, body, "indicators_found")
    ...
    def _parse_entities(self, param):
        ...
        resp_status, response = self._make_rest_call_helper(consts.SIM_PARSE_ENTITIES_ENDPOINT, action_result, data=payload, method="post")
    ...
    def _list_observable_types(self, param):
        ...
        for observable_type in consts.SIM_OBSERVABLE_TYPES:
            type = dict()
            type["observable_type"] = observable_type
            action_result.add_data(type)

        summary_data["observable_type_count"] = len(consts.SIM_OBSERVABLE_TYPES)
        self.debug_print("Observable type count is {}".format(summary_data["observable_type_count"]))

        return action_result.set_status(phantom.APP_SUCCESS)

    def _triage_email(self, param):
        """Change the status of a Splunk Intelligence Management phishing triage submission
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(
            consts.SIM_TRIAGE_SUBMISSION_ENDPOINT.format(submission_id=email), action_result, params=params, method="post"
        )
    ...
    def _get_report(self, param):
        """Return the raw report data, extracted indicators and other metadata for a Splunk Intelligence Management report
         given its report id.
        ...
        report_id = param[consts.SIM_JSON_REPORT_ID]

        # Optional parameters
        id_type = param.get(consts.SIM_JSON_REPORT_ID_TYPE, "internal")
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(
            consts.SIM_GET_REPORT_ENDPOINT.format(report_id=report_id), action_result, params=query_param, method="get"
        )
    ...
    def _copy_report(self, param):
        ...
        # Mandatory parameters
        report_id = param[consts.SIM_JSON_REPORT_ID]
        dest_enclave_id = param[consts.SIM_JSON_DEST_ENCLAVE]
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(
            consts.SIM_COPY_REPORT_ENDPOINT.format(report_id=report_id), action_result, params=query_param, method="post"
        )
    ...
    def _move_report(self, param):
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(
            consts.SIM_MOVE_REPORT_ENDPOINT.format(report_id=report_id), action_result, params=query_param, method="post"
        )
    ...
    def _normalize_timestamp(self, date_time):
        """Attempt to convert a string timestamp in to a Splunk Intelligence Management compatible format for submission.
    ...
    def _delete_report(self, param):
        """Delete a Splunk Intelligence Management report
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(
            consts.SIM_GET_REPORT_ENDPOINT.format(report_id=report_id), action_result, params=query_param, method="delete"
        )
    ...
    def _submit_report(self, param):
        """Submit a report to community or enclaves and returns its Splunk Intelligence Management report ID and
        ...
        # Mandatory parameters
        report_title = param[consts.SIM_JSON_REPORT_TITLE]
        report_body = param[consts.SIM_JSON_REPORT_BODY]
        distribution_type = param[consts.SIM_JSON_DISTRIBUTION_TYPE]

        # Optional parameters
        enclave_ids = param.get(consts.SIM_JSON_ENCLAVE_IDS)
        time_discovered = param.get(consts.SIM_JSON_TIME_DISCOVERED)
        external_tracking_id = param.get(consts.SIM_JSON_TRACKING_ID)
        external_url = param.get(consts.SIM_JSON_EXTERNAL_URL)

        # Normalize timestamp
        report_time_began = self._normalize_timestamp(time_discovered)
        if not report_time_began:
            return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_TIME_FORMAT)
        ...
        if distribution_type == "ENCLAVE":
            ...
            if not (enclave_ids or self._config_enclave_ids):
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_ERR_MISSING_ENCLAVE_ID)
            ...
            if self._config_enclave_ids:
                ...
                config_enclave_id_list = list(
                    filter(lambda x: x.strip(), [config_enclave_id.strip() for config_enclave_id in config_enclave_id_list])
                )
            ...
            if set(enclave_id_list) - set(config_enclave_id_list):
                return action_result.set_status(phantom.APP_ERROR, consts.SIM_UNKNOWN_ENCLAVE_ID)
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(
            consts.SIM_SUBMIT_REPORT_ENDPOINT, action_result, json=submit_report_payload, method="post"
        )
        ...
        if not re.findall("^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$", response):
            return action_result.set_status(phantom.APP_ERROR, consts.SIM_BAD_REPORT_ID.format(response=response))
    ...
    def _update_report(self, param):
        """Grab a report from Splunk Intelligence Management and update the fields with the
        ...
        # Optional parameters
        id_type = param.get(consts.SIM_JSON_REPORT_ID_TYPE, "internal")
        report_title = param.get(consts.SIM_JSON_REPORT_TITLE)
        report_body = param.get(consts.SIM_JSON_REPORT_BODY)
        enclave_ids = param.get(consts.SIM_JSON_ENCLAVE_IDS)
        time_discovered = param.get(consts.SIM_JSON_TIME_DISCOVERED)
        external_tracking_id = param.get(consts.SIM_JSON_TRACKING_ID)
        external_url = param.get(consts.SIM_JSON_EXTERNAL_URL)
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(
            consts.SIM_UPDATE_REPORT_ENDPOINT.format(report_id=report_id), action_result, json=payload, method="put"
        )
    ...
    def _safelist_ioc(self, param):
        """Add the provided IOC to the Splunk Intelligence Management whitelist.
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(consts.SIM_WHITELIST_ENDPOINT, action_result, json=body, method="post")
    ...
    def _unsafelist_ioc(self, param):
        """Remove the provided IOC from the Splunk Intelligence Management whitelist.
        ...
        # Get mandatory parameters
        ioc = param[consts.SIM_HUNT_IOC_PARAM]
        ioc_type = param[consts.SIM_IOC_TYPE_PARAM]

        params = {"indicatorType": ioc_type, "value": ioc}

        # Make REST call
        resp_status, response = self._make_rest_call_helper(consts.SIM_WHITELIST_ENDPOINT, action_result, params=params, method="delete")
    ...
    def _list_enclaves(self, param):
        """List all the enclaves in Splunk Intelligence Management
        ...
        # Make REST call
        resp_status, response = self._make_rest_call_helper(consts.SIM_ENCLAVES_ENDPOINT, action_result)
...
if __name__ == "__main__":
    ...
    if args.username and args.password:
        ...
        try:
            ...
            r = requests.get(login_url, verify=verify, timeout=consts.SIM_DEFAULT_TIMEOUT)
            csrftoken = r.cookies["csrftoken"]
            data = {"username": args.username, "password": args.password, "csrfmiddlewaretoken": csrftoken}
            headers = {"Cookie": "csrftoken={0}".format(csrftoken), "Referer": login_url}

            print("Logging into Platform to get the session id")
            r2 = requests.post(login_url, verify=verify, data=data, headers=headers, timeout=consts.SIM_DEFAULT_TIMEOUT)
